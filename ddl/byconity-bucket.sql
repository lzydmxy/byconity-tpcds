...
) ENGINE=CnchMergeTree() ORDER BY (ca_address_sk) CLUSTER BY(ca_address_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (cd_demo_sk) CLUSTER BY(cd_demo_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (d_date_sk) CLUSTER BY(d_date_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (w_warehouse_sk) CLUSTER BY(w_warehouse_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (sm_ship_mode_sk) CLUSTER BY(sm_ship_mode_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (t_time_sk) CLUSTER BY(t_time_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (r_reason_sk) CLUSTER BY(r_reason_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (ib_income_band_sk) CLUSTER BY(ib_income_band_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (i_item_sk) CLUSTER BY(i_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (s_store_sk) CLUSTER BY(s_store_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (cc_call_center_sk) CLUSTER BY(cc_call_center_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (c_customer_sk) CLUSTER BY(c_customer_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE=CnchMergeTree() ORDER BY (web_site_sk) CLUSTER BY(web_site_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (sr_item_sk, sr_ticket_number) CLUSTER BY(sr_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (hd_demo_sk) CLUSTER BY(hd_demo_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (wp_web_page_sk) CLUSTER BY(wp_web_page_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (p_promo_sk) CLUSTER BY(p_promo_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (cp_catalog_page_sk) CLUSTER BY(cp_catalog_page_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (inv_date_sk, inv_item_sk, inv_warehouse_sk) CLUSTER BY(inv_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (cr_item_sk, cr_order_number) CLUSTER BY(cr_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (wr_item_sk, wr_order_number) CLUSTER BY(wr_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (ws_item_sk, ws_order_number) CLUSTER BY(ws_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (cs_item_sk, cs_order_number) CLUSTER BY(cs_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
...
) ENGINE = CnchMergeTree() ORDER BY (ss_item_sk, ss_ticket_number) CLUSTER BY(ss_item_sk) INTO __BUCKET_SIZE__ BUCKETS SETTINGS enable_nullable_sorting_key=1;
